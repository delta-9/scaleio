...
const { IgeEventingClass } = window;
/**
 * When added to a viewport, automatically adds mouse panning
 * capabilities to the viewport's camera.
 */
export var MouseMoveCameraComponent = IgeEventingClass.extend({
	classId: 'MouseMoveCameraComponent',
	componentId: 'mouseMoveCamera',
	...
	enabled: function (val) {
		...
		if (val !== undefined) {
			...
			if (this._enabled) {
				...
				var curMousePos = window.ige._mousePos;
	...
	_mouseMove: function (event) {
		if (this._enabled) {
			...
			//if (this._panStartMouse) {
				...
				if ((panFinalX > 400 || panFinalX < -400) || (panFinalY > 200 || panFinalY < -200)) {
